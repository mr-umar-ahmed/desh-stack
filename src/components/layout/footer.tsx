...
export function Footer() {
  return (
    <footer className="aurora grain relative w-full overflow-hidden border-t border-indigo/10 bg-paper-2 text-ink/80">
      ...
      <div className="relative z-10 container mx-auto grid grid-cols-1 gap-10 px-4 py-16 md:grid-cols-4 md:px-6">
        ...
        <div className="flex flex-col gap-4">
          <Link
            href="/"
            className="font-heading flex items-center gap-2.5 text-2xl font-black tracking-tight text-ink"
          >
            <span className="logo-tile flex h-9 w-9 items-center justify-center rounded-xl text-sm font-black text-ink shadow-md">
              D
            </span>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3">
            <span className="flex items-center gap-1.5 rounded-full border border-white/10 bg-card/5 px-3 py-1 text-xs text-ink/60">
          ...
          <nav key={group.heading} className="flex flex-col gap-3">
            ...
            {group.links.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="w-fit text-sm text-ink/60 transition-colors hover:text-saffron"
              >
                {link.label}
              </Link>
            ))}
      ...
      <div className="relative z-10 container mx-auto flex flex-col items-center justify-between gap-4 border-t border-ink/10 px-4 py-6 md:flex-row md:px-6">
